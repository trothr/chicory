...
#		make rules for GLIBC at La Casita
#	  Date: 2008-Apr-05 (Sat) Project Omaha
#
#
...
# no default
#SC_URL		=  http://ftp.gnu.org/pub/gnu/glibc/glibc-2.3.3.tar.gz \
#        http://ftp.gnu.org/pub/gnu/glibc/glibc-linuxthreads-2.3.3.tar.gz
SC_URL		=  http://ftp.gnu.org/pub/gnu/$(APPLID)/$(SC_VRM).tar.gz

# defaults
SC_FETCH	=	wget --passive-ftp --no-clobber $(SC_URL)
SC_FETCH_BZ	=	wget --passive-ftp --no-clobber $(SC_URL) ; \
	  bzcat $(SC_SOURCE).tar.bz2 | gzip > $(SC_SOURCE).tar.gz ; \
		touch -r $(SC_SOURCE).tar.bz2 $(SC_SOURCE).tar.gz
#SC_FETCH	=	wget --passive-ftp --no-clobber $(SC_URL) ; \
#			./$(SC_SOURCE)-fetch.sh $(SC_SOURCE) $(SC_URL)
