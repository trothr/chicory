...
#		make rules for Tcl with /usr/opt at La Casita
#	  Date: 2015-Feb-27 (Sun) Project Omaha and NORD on FreeBSD
#
...
#SC_URL		=	\
# http://prdownloads.sourceforge.net/$(APPLID)/$(SC_SOURCE)-src.$(SC_ARC)
SC_URL		=	\
	      ftp://ftp.tcl.tk/pub/tcl/tcl8_5/$(SC_SOURCE)-src.$(SC_ARC)
...
#SC_BUILD	=	cd unix ; $(MAKE)
#SC_INSTALL	=	cd unix ; $(MAKE) install

# default for this is blank, varies widely per package
#SC_FIXUP	=	strip ...
SC_FIXUP	=	cd bin ; strip tclsh* ; ln -s tclsh* tclsh
...
#
#
$(SC_SOURCE):	makefile arc/$(SC_SOURCE)-src.tar.gz
		rm -f src _src ; rm -rf $(SC_SOURCE)
		tar xzf arc/$(SC_SOURCE)-src.tar.gz
		test -d $(SC_SOURCE)
		ln -s $(SC_SOURCE) src
#		@test -x repatch.sh
...
#
#
archive:	arc/$(SC_SOURCE)-src.tar.gz

#
# retrieve source from Internet
arc/$(SC_SOURCE)-src.tar.gz:
#		@test ! -z "$(APPLID)"
#		@echo "$(MAKE): need source for '$(APPLID)' ..."
		...
		@test ! -z "$(SC_SOURCE)"
		@test ! -z "$(SC_URL)"
		@rm -f arc/$(SC_SOURCE)-src.tar.gz
		@mkdir -p arc
		@test -d arc
		sh -c ' cd arc ; $(SC_FETCH) '
		@test -s arc/$(SC_SOURCE)-src.tar.gz
#		@test -s arc/$(SC_VRM)-src.tar.gz
...
#
#
$(APPLID).src:	arc/$(SC_VRM)-src.tar.gz
		@test ! -z "$(APPLID)"
		@test ! -z "$(SC_VRM)"
		@test ! -z "$(SC_SOURCE)"
