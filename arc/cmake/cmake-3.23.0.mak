...
#		make rules for CMake at La Casita with Chicory
#	  Date: 2022-Apr-08 (Fri) for Eric la Franchi
#
...
#SC_TAR		=	(bzcat - | tar xf -) <
#SC_TAR		=	tar xJf
#SC_TAR		=	(xzcat - | tar xf -) <          ### needs testing
#SC_TAR		=	tar --lzip -xf
#SC_TAR		=	(lzip -d | tar xf -) <

# where to find the source on the internet (no default)
SC_URL		=	\
	   http://www.cmake.org/files/v3.23/$(SC_SOURCE).$(SC_ARC) \
	   http://www.cmake.org/files/v3.23/$(SC_SOURCE)-SHA-256.txt \
	   http://www.cmake.org/files/v3.23/$(SC_SOURCE)-SHA-256.txt.asc

#SC_SOURCE_VERIFY = gpg --verify arc/$(SC_SOURCE).$(SC_ARC).asc
SC_SOURCE_VERIFY = gpg --verify arc/$(SC_SOURCE)-SHA-256.txt.asc \
	&& sha256sum arc/$(SC_SOURCE).$(SC_ARC) \
	| awk '{print "grep" , $$1 , "arc/$(SC_SOURCE)-SHA-256.txt"}' \
	| sh
#gpg --keyserver hkp://pool.sks-keyservers.net/ --recv-keys 0x2d2cef1034921684
