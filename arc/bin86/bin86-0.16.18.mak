...
#		make rules for building BIN86 with /usr/opt
#	  Date: 2014-Jul-27 (Sun) Project Omaha and NORD
#
...
SC_URL		=	\
 http://distro.ibiblio.org/slitaz/sources/packages-stable/b/$(SC_VRM).tar.gz
#SC_SOURCE_VERIFY = gpg --verify arc/$(SC_VRM).tar.gz.asc
...
SC_BUILDX	=		$(MAKE) "PREFIX=$(PREFIX)/$(SC_VRM)"
...
#
		...
		rm -f $(SYSTEM)
		mkdir "$(SYSTEM)"
		@mkdir "$(SYSTEM)/bin" "$(SYSTEM)/man"
		@mkdir "$(SYSTEM)/man/man1"
		sh -c ' cd "$(SYSTEM)" ; exec \
			ln -s `pwd` "$(PREFIX)/$(SC_VRM)" '
#
		@echo "$(MAKE): post-building '$(SC_VRM)' for '$(SYSTEM)' ..."
		sh -c ' cd $(SC_SOURCE) ; exec $(SC_BUILDX) install ' \
			2>&1 | tee install.log
		echo "$(SYSTEM)" > _ins
		rm "$(PREFIX)/$(SC_VRM)"
		if [ ! -z "$(SC_FIXUP)" ] ; then \
